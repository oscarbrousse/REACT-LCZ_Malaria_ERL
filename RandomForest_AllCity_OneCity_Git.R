...
for (city in cities){
  ...
  for(j in 1:bootstrap) {
    ...
    city_train=subset(data_city, !(X %in% city_test$X))
  ...
  write.csv(var_importance, file = paste0("Final_Merged_Predictors/Leave_One_City/",city,"_",res,"_Var_Importance",Norm,filt,"_AC.csv"))
  write.csv(results, file = paste0("Final_Merged_Predictors/Leave_One_City/",city,"_",res,"_RF_Results",Norm,filt,"_AC.csv"))
}
